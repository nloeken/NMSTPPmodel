...
class NMSTPP_Sequential(nn.Module):
    def __init__(self, cont_dim, dim_360, seq_len, 
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # NEU
        ...
        # MODIFIZIERT: Erlerntes Positional Embedding wird entfernt.
        # self.pos_emb = nn.Parameter(torch.randn(seq_len, d_model) * 0.02)
        
        # MODIFIZIERT: batch_first=True ist einfacher zu handhaben (wie im Paper)
        ...
        self.zone_net = nn.Sequential(
            nn.Linear(d_model + 1, hidden_dim), # Input: Transformer-Output + Zeit-Vorhersage
            nn.ReLU(), 
            nn.Dropout(dropout_rate),
            nn.Linear(hidden_dim, num_zones)
        )
        
        # MODIFIZIERT: Target Zone Embedding wird nicht mehr benötigt
        # self.target_zone_emb = nn.Embedding(num_zones, zone_emb_dim)
        
        # MODIFIZIERT: Input-Größe angepasst an die neue Logik (Transformer-Out + Zeit-Out + Zonen-Logits)
        self.action_net = nn.Sequential(
            nn.Linear(d_model + 1 + num_zones, hidden_dim), 
            nn.ReLU(), 
            nn.Dropout(dropout_rate),
            nn.Linear(hidden_dim, num_actions)
        )

    # NEU: Feste Sinus/Kosinus Positions-Codierung aus dem Paper
    def positional_encoding(self, src):
        ...
        # Füge Batch-Dimension hinzu und wiederhole für jeden Batch
        return pos_encoding.unsqueeze(0).repeat(batch_size, 1, 1)


    def forward(self, x_cat, x_cont, x_360=None): # Teacher Forcing wird nicht mehr benötigt
        ...
        # MODIFIZIERT: Feste Positions-Codierung anwenden
        x = x + self.positional_encoding(x)

        # MODIFIZIERT: Keine Permutation mehr nötig wegen batch_first=True
        x = self.transformer(x)
        x = x[:, -1, :] # Nimm den Output des letzten Zeitschritts

        # ---- Autoregressiver Prediction Head (wie im Paper) ----
        time_pred = self.time_net(x) # Shape: (B, 1)
        
        zone_input = torch.cat([x, time_pred], dim=-1)
        zone_logits = self.zone_net(zone_input) # Shape: (B, num_zones)
        
        # MODIFIZIERT: Kombiniere Transformer-Output, Zeit-Output UND Zonen-Logits
        action_input = torch.cat([x, time_pred, zone_logits], dim=-1)
        action_logits = self.action_net(action_input) # Shape: (B, num_actions)
    ...
    def loss(self, preds, targets, action_weights=None):
        ...
        # HINWEIS: Hier kannst du die Gewichte anpassen, wie zuvor besprochen!
        #total_loss = 10.0 * l_time + l_zone + 2.0 * l_action #alt
        total_loss = 10.0 * l_time + 1.2 * l_zone + 1.5 * l_action
