...
def make_dataloaders(X, y, le_action, batch_size): # batch_size ist jetzt ein Parameter
    y_act = y['act']
    ...
    num_samples = len(ds)
    split_idx = int(num_samples * 0.8) # 80/20 Split
    all_indices = np.arange(num_samples)

    # Die Daten werden NICHT gemischt, bevor sie aufgeteilt werden.
    tr_idx = all_indices[:split_idx]
    va_idx = all_indices[split_idx:]
    print(f"Chronologischer Split: {len(tr_idx)} Trainings-, {len(va_idx)} Validierungsbeispiele.")
    # Shuffle=True im train_loader ist weiterhin sinnvoll, um die Trainingsdaten in jeder Epoche zu mischen.
    train_loader = DataLoader(torch.utils.data.Subset(ds, tr_idx), batch_size=batch_size, shuffle=False, drop_last=True, num_workers=0) #shuffle noch auf true setzen
...
def train(model, train_loader, val_loader, action_weights, lr, weight_decay): # lr und weight_decay sind jetzt Parameter
    ...
    for ep in range(1, EPOCHS + 1):
        model.train()
        # ... (Trainingsschleife bleibt gleich)
        train_loss = 0
...
# MODIFIZIERT: Die Funktion gibt jetzt den Macro F1-Score zurück
def evaluate(model, loader, le_action_dict):
    ...
    with torch.no_grad():
        for xc, xcont, x360, yz, ya, yt in loader:
            ...
            _, alog, _ = model(xc, xcont, x360, targets=None, teacher_forcing=False)
    ...
    # Gib den classification report als Text aus
...
# NEU: Die objective-Funktion für Optuna
def objective(trial: optuna.Trial):
    """
    Diese Funktion führt einen kompletten Trainings- und Evaluationslauf mit
    einem Satz von Hyperparametern durch, die von Optuna vorgeschlagen werden.
    """
    ...
    print(f"\n===== Starte Trial {trial.number} mit folgenden Parametern =====")
    ...
    # Daten werden für jeden Trial neu geladen, um saubere Bedingungen zu schaffen
    ...
    model = NMSTPP_Sequential(
        ...
        d_model=d_model,
        nhead=8 if d_model % 8 == 0 else 4, # nhead muss Teiler von d_model sein
    ...
    # --- 5. Evaluation und Rückgabe des Zielwerts ---
...
def main():
    # NEU: Die main-Funktion startet und verwaltet die Optuna-Studie
    
    # Anzahl der Versuche, die Optuna durchführen soll
    n_trials = 50 
    
    print(f"Starte Optuna-Studie für {n_trials} Trials...")
    
    # Wir wollen den Macro F1-Score maximieren
    study = optuna.create_study(direction='maximize')
    
    # Starte die Optimierung. `objective` wird n_trials mal aufgerufen.
